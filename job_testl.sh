...
for target_directory in "$source_directory"/*; do
    if [ -d "$target_directory" ]; then
        ...
        python3 ./test.py --input_path "$source_directory/" --output_path "./outputs/freebase-large/" --suffix "$name" --threshold 0.3
    fi
    exit
done

# source_directory="./inputs/pharmacogenomics"

# for target_directory in "$source_directory"/*; do
#     if [ -d "$target_directory" ]; then
#         name=$(basename "$target_directory")
#         python3 ./test.py --input_path "$source_directory/" --output_path "$source_directory/" --suffix "$name" --threshold 0.0
#     fi
#     exit
# done
