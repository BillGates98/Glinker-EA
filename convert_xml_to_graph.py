...
class Convertor:
    ...
    def get_pairs_entites(self, graph_source=None, entities1=[], graph_target=None, entities2=[]):
        outputs = []
        outputs_entities1 = self.get_entities(
            graph=graph_source, entities=entities1, dsource='source')
        outputs_entities2 = self.get_entities(
            graph=graph_target, entities=entities2, dsource='target')
        outputs_products = self.cartesian_product(
            set1=outputs_entities1, set2=outputs_entities2)
        # parallel computing
        pool = multiprocessing.Pool()
        result_async = [pool.apply_async(self.find_valid_pairs, args=(
            chunked_data, entities1, entities2, )) for chunked_data in self.split_arrays(a=outputs_products)]
    ...
    def save_entityies_to_graph(self, alignments=[], file_name=''):
        ...
        for entity1, entity2 in alignments:
            ...
            query = query.replace('?subject', entity1).replace(
                '?predicate',  _predicate).replace('?object',  entity2)
            query_result = None
            try:
                ...
                continue
        graph.serialize(destination=self.destination +
                        file_name + '.ttl', format='turtle')
        return graph

    def run(self):
        pattern_matching = self.convert()
        self.save_entityies_to_graph(
            alignments=pattern_matching, file_name='valid_same_as')
...
if __name__ == '__main__':
    def arg_manager():
        ...
        parser.add_argument("--input_data", type=str,
                            default='./conversions/doremus/data_doremus.xml')
        # parser.add_argument("--input_path", type=str, default='./inputs/doremus/')
        parser.add_argument("--output_path", type=str,
                            default="./validations/doremus/")
        return parser.parse_args()
    args = arg_manager()
    Convertor(input_path='', input_file=args.input_data,
              destination=args.output_path).run()

# python3.8 ./convert_xml_to_graph.py --input_data ./conversions/spimbench2/refalign.xml --output_path ./validations/spimbench2/
